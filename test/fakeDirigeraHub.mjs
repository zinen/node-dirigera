...
async function fakeHub(debugLvl) {
  ...
  const debugLevel = debugLvl ? debugLvl : 0
  ...
  return new Promise(async (resolve, reject) => {
    ...
    const server = https.createServer(credentials, async (req, res) => {
      ...
      if (requestedPath === '/') {
        ...
        const failedChecks = []
        if (queryData.audience != 'homesmart.local') failedChecks.push('Query must contain audience=homesmart.local')
        if (queryData.response_type != 'code') failedChecks.push('Query must contain response_type=code')
        if (queryData.challenge_method != 'S256') failedChecks.push('Query must contain challenge_method=S256')
        if (!queryData.code_challenge || queryData.code_challenge.length < 20) failedChecks.push('Query must contain code_challenge= a string longer then 20')
